  ...
  enterInspectionMode(selectedRoot, selectedCompData, allGenComponents) {
    ...
    allGenComponents.forEach((compData) => {
      ...
      if (compData.group === selectedRoot) {
        ...
        // The Fusion scene is converted Z-up -> Three.js Y-up, so local Z is the visual
        // vertical axis. A generous lift turns selection into an unmistakable exploded-view
        // reveal instead of a subtle camera-only focus.
        const extractionForwardZ = 4.80;
        const extractionOutwardX = normOutX * 0.85;
        const extractionLiftY = 1.05 + Math.max(0, normOutY * 0.55);
